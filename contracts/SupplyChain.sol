...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint sku) {
    // require(items[_sku].name != 0);
    require(bytes(items[sku].name).length != 0);
    require(items[sku].price != uint(0));
    // require(items[_sku].seller != 0x0);
    require(items[sku].state == uint(State.ForSale));
    _;
  }
  ...
  function buyItem(uint sku) 
    payable public forSale(sku) paidEnough(items[sku].price) checkValue(sku)
  {
    items[sku].buyer = msg.sender; //set buyer as msg.sender
